...
let SignupUser = async function (req, res, next) {
    try {
        const { username, email, password , BookId } = req.body;

        const hashedPassword = await bcrypt.hash(password, 10);

        const CreateUser = await User.create({ username, email, password: hashedPassword , BookId});
...
let ReadAllUser = async function (req, res, next) {
    try {
        ...
        const ReadUsers = await User.find().populate('BookId');
        res.status(201).json({
            status: "success",
...
let UpdateUser = async function (req, res, next) {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(id, req.body, { new: true }).populate('BookId');
        if (!updatedUser) {
            return res.status(404).json({ message: 'User not found' });
        }
        res.status(201).json({
